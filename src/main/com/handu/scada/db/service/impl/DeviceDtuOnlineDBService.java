 ...
 */
public class DeviceDtuOnlineDBService extends BaseDBService implements IDBService {
    ...
    protected String startSql() {
        return " insert into device_staterecord(RecordId,DeviceTableName,DeviceId,State,Description,OnlineTime) values ";
    }

    @Override
    protected String endSql() {
        return " on duplicate key update RecordId=values(RecordId),DeviceTableName=values(DeviceTableName),DeviceId=values(DeviceId),State=values(State),Description=values(Description),OnlineTime=values(OnlineTime)";
    }
